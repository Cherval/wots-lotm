<template>
  <div class="animate-fade-in">
    <div class="flex justify-between items-center mb-6 bg-vic-darkbrown p-4 rounded border border-vic-brown">
      ...
      <button v-if="isAdmin" @click="emit('createMap')" class="btn-gold text-sm">+ สร้างแผนที่</button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="map in maps" :key="map.id" class="group relative bg-black border-2 border-vic-brown rounded-lg overflow-hidden shadow-lg hover:border-vic-gold transition">
        ...
        <!-- Lock Badge -->
        <div v-if="map.is_locked" class="absolute top-2 right-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded shadow border border-red-400">
          🔒 ล็อค
        </div>
        ...
        <div class="p-4 relative bg-vic-darkbrown">
          ...
          <div class="flex gap-2">
            ...
            <button v-if="isAdmin" @click="emit('editMap', map)" class="px-3 bg-blue-900 text-blue-200 rounded border border-blue-700 hover:bg-blue-800">
              ✏️
            </button>
            <button v-if="isAdmin" @click="emit('deleteMap', map.id)" class="px-3 bg-red-900 text-red-200 rounded border border-red-700 hover:bg-red-800">
              🗑️
            </button>
          </div>
        </div>
